...
export default function Vizion2() {
  ...
  if(url && playerRef.current) {
    ...
    playerRef.current.play().catch(e => console.log("Audio error:", e));
    playerRef.current.ontimeupdate = () => setProg(Math.floor(playerRef.current!.currentTime));
  ...
  return(
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,color:C.text,height:"100vh",display:"flex",flexDirection:"column",overflow:"hidden",transition:"background .4s"}}>
          ...
          <div style={{padding:pad,maxWidth:1000,margin:"0 auto",width:"100%",animation:"fadeUp .35s ease both"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:14}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                {filtered.map(a=>(
                  <div key={a.id} className="ch" style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:13,overflow:"hidden",cursor:"pointer",transition:"all .25s"}}>
      ...
      {/* PLAYER */}<audio ref={playerRef}
       onTimeUpdate={e=>setProg(Math.floor((e.target as HTMLAudioElement).currentTime))} onEnded={()=>setPlay(false)}/>{playing&&(
  ...
  ()=>{ playing ? playerRef.current?.pause() : playerRef.current?.play(); setPlay(pp=>!pp); }
